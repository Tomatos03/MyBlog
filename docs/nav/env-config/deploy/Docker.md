...
> [!NOTE]
> 端口映射仅仅是宿主机->容器的单向映射，容器内的服务不能直接访问宿主机的端口。
...
在 Docker 中，默认的端口映射是单向的，即宿主机的端口可以访问容器的端口，但容器无法直接访问宿主机的端口。要实现双向端口通信，可以通过以下方式配置：

#### 方式一

通过 `--add-host` 参数将宿主机的 IP 地址映射到容器和宿主机的网关地址，从而使容器可以通过该域名访问宿主机的端口：

```bash
# 添加宿主机映射
# host-gateway 是 容器与宿主机之间的网关地址
# host-docker.internal 指向宿主机的 IP 地址
...
> [!NOTE]
> 这个命令参数实际上是在容器的 `/etc/hosts` 文件中添加了一条记录，将 `host.docker.internal` 映射为容器和宿主机的网关地址。

#### 注意事项

1. **宿主机 IP 地址**：在 Linux 系统中，宿主机的默认 IP 地址通常是 `172.17.0.1`，可以通过 `ip addr` 命令查看。
2. **防火墙配置**：确保宿主机的防火墙规则允许容器访问宿主机的端口。
3. **网络模式**：如果使用 `host` 网络模式，容器会与宿主机共享网络堆栈，从而实现双向通信，但会牺牲隔离性。

> [!TIP]
> 使用 `--add-host` 是一种简单且高效的方式，适合大多数场景。如果需要更复杂的网络配置，可以结合自定义网络和防火墙规则。
